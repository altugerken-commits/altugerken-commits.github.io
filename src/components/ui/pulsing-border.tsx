...
//
// ---------------------------------------------------------------------------
// LOCAL NOTES (this repo)
//
// 1. "use client" above is a Next.js App Router directive and is inert in
//    Astro, which decides hydration at the call site with client:* instead.
//    It is kept so the file stays byte-comparable with upstream when re-pulled.
//
// 2. The ONLY edit to upstream is the UNIFORMS colour recipe — see the comment
//    on that block. The shader source and the whole render loop are untouched.
//
// 3. The fragment shader writes alpha 1.0, so this canvas is OPAQUE. Mounted
//    over the site's Three.js stage it would hide it completely. The call site
//    composites it with mix-blend-mode: screen, which is why colour 0 is pure
//    black below: under screen, black is a no-op and only the lit border adds.
// ---------------------------------------------------------------------------
...
const UNIFORMS = {
  // THE ONLY DEPARTURE FROM UPSTREAM.
  //
  // The shipped recipe is a blue/cyan ramp:
  //   [0.0196,0.0275,0.0510] near-black blue
  //   [0.0706,0.3804,0.6275] #1261A0 blue
  //   [0.2078,0.7686,0.9098] #35C4E8 cyan
  //   [0.9490,0.9843,1.0000] #F2FBFF near-white
  // Restore those four values to get the original look back.
  //
  // Two reasons it is retuned rather than kept:
  //
  // 1. COMPOSITING. Colour 0 is the field the border sits in, and the shader
  //    writes alpha 1.0. The call site blends this canvas with
  //    mix-blend-mode: screen so the Three.js stage behind it survives; screen
  //    leaves black untouched and adds everything else, so colour 0 must be
  //    EXACTLY black or the whole hero gets lifted by a flat dark-blue wash.
  //
  // 2. PALETTE. The site runs a strictly monochrome-warm light system
  //    (--color-glow #ffd9a8, --color-oxide #e08a55, --color-ink #f6f5f2) and
  //    a cyan frame would be the one cool object on the page. These three are
  //    those tokens converted to 0..1 float.
  colors: [
    [0, 0, 0],
    [0.8784313725490196, 0.5411764705882353, 0.3333333333333333],
    [1, 0.8509803921568627, 0.6588235294117647],
    [0.9647058823529412, 0.9607843137254902, 0.9490196078431372],
    [0.9647058823529412, 0.9607843137254902, 0.9490196078431372],
    [0.9647058823529412, 0.9607843137254902, 0.9490196078431372],
    [0.9647058823529412, 0.9607843137254902, 0.9490196078431372],
    [0.9647058823529412, 0.9607843137254902, 0.9490196078431372],
  ...
  vignette: 0.000,
  blur: 0.0000,
  // Upstream 0.042. Zeroed here: under mix-blend-mode: screen this grain is
  // ADDITIVE, so it would lift the entire hero by up to +0.042 luminance as a
  // visible haze — and the page already lays its own film grain over
  // everything at .grain.
  grain: 0.000,
  seed: 1.0,
  rotate: 0.0000,
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // UPSTREAM FIX (2 of 2). These two were `function` declarations, which are
    // hoisted to the top of this effect body. TypeScript therefore refuses to
    // carry the `if (!canvas) return` / `if (!gl) return` narrowing into them —
    // a hoisted function is in scope before those guards run, so from the
    // checker's view it could be called while both are still null. That is 7
    // errors under Astro's strict config, on code that is correct at runtime.
    //
    // As arrow consts they cannot be referenced before their initialiser, so
    // the narrowing holds and the guards do their job. Behaviour is identical:
    // neither is called before it is defined, and the mutual recursion between
    // them still works because the reference is inside a body, not an
    // initialiser.
    const requestRender = () => {
